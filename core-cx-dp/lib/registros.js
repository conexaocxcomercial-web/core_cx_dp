...
import { COLUNA_CLIENTE, clienteAtual, exigirColunaCliente, mesmoCliente } from '@/lib/contexto';
...
/** Lê a aba e devolve só o que pertence à empresa de quem está logado. */
const lerEscopo = cache(async (aba) => {
  const [{ cabecalho, registros }, cliente] = await Promise.all([lerAba(aba), clienteAtual()]);

  exigirColunaCliente(cabecalho, aba);
...
async function gravar(aba, valoresPorColuna) {
  ...
  const completo = { ...valoresPorColuna, [COLUNA_CLIENTE]: cliente };
