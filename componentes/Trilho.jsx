...
export function Trilho({ usuario, sair }) {
    ...
    <>
      ...
      <aside className="sobre-escuro fixed inset-y-0 left-0 z-30 hidden w-[232px] flex-col bg-tinta lg:flex">
        <div className="px-6 pb-7 pt-7">
          ...
          <p className="mt-2 truncate text-[12px] leading-snug text-folha/55">
            {usuario?.empresa || 'Registro de pessoal'}
          </p>
        </div>
        ...
        <div className="border-t border-folha/12 px-6 py-5">
          <p className="truncate text-[13px] font-medium text-folha">{usuario?.nome}</p>
          <p className="mt-0.5 truncate text-[11.5px] text-folha/50">{usuario?.email}</p>
          <form action={sair} className="mt-3">
            <button
              type="submit"
              className="text-[12.5px] text-folha/60 underline-offset-4 transition-colors hover:text-folha hover:underline"
            >
              Sair
            </button>
          </form>
      ...
      <header className="sobre-escuro sticky top-0 z-30 bg-tinta lg:hidden">
        <div className="flex items-center justify-between px-5 py-3.5">
          ...
          <form action={sair}>
            <button type="submit" className="text-[12.5px] text-folha/65">
              Sair
            </button>
          </form>
        </div>
