...
import { abrirSessao, encontrarUsuario, fecharSessao, listarUsuarios } from '@/lib/sessao';
import {
...
export async function entrar(_estado, dados) {
  ...
  const destino = ler(dados, 'destino') || '/painel';
  ...
  if (listarUsuarios().length === 0) {
    return {
      erro: 'Nenhum acesso foi configurado ainda. Defina AUTH_USUARIOS nas variáveis de ambiente.',
    };
  }

  const usuario = encontrarUsuario(email, senha);

  if (!usuario) {
    return { erro: 'E-mail ou senha não conferem. Confira os dados e tente de novo.' };
  }

  if (!usuario.cliente) {
    return {
      erro: 'Este acesso não está ligado a nenhuma empresa. Acrescente o campo "cliente" a este usuário em AUTH_USUARIOS.',
    };
  }
